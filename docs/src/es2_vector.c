 ...
 *
 * L'obiettivo è verificare che il vector clock rilevi correttamente:
 ...
 *    0  se a == b (identici)
 *    2  se a || b (concorrenti: né a<=b né b<=a)
 *
 * Usa le definizioni:
...
static void *node0(void *arg) {
    (void)arg;
    vc_event(0);  vc_copy(ts_a, 0);        /* evento interno a     */
    vc_send(0, 1, 42);
    vc_copy(ts_send_p0, 0);                 /* salva dopo R2        */
    vc_event(0);                            /* evento interno b     */
    return NULL;
}

static void *node1(void *arg) {
    (void)arg;
    vc_event(1);  vc_copy(ts_c, 1);        /* evento interno c     */
    vc_recv(1);   vc_copy(ts_recv_p1, 1);  /* riceve da P0         */
    vc_send(1, 2, 0);
    vc_copy(ts_send_p1, 1);                 /* salva dopo R2        */
    return NULL;
}

static void *node2(void *arg) {
    (void)arg;
    vc_recv(2);  vc_copy(ts_recv_p2, 2);   /* riceve da P1         */
    vc_event(2);                            /* evento interno d     */
    return NULL;
}
 ...
 * TODO 5 — verify()
 *
 * Chiama vc_cmp sulle coppie seguenti e stampa il risultato:
 *
 *   (a) ts_a       vs ts_c        → atteso: CONCORRENTI
 *   (b) ts_send_p0 vs ts_recv_p1  → atteso: CAUSALE (send → recv)
 *   (c) ts_send_p1 vs ts_recv_p2  → atteso: CAUSALE (send → recv)
 *
 * Per ciascuna coppia stampa PASS se il risultato coincide con
 * l'atteso, FAIL altrimenti.
 * ════════════════════════════════════════════════════════════════════ */
static void verify(void) {
    printf("\n=== Confronti vector clock ===\n");
    /* TODO */
    (void)cmp_str;   /* rimuovi questo cast quando usi cmp_str */
    (void)vc_print;
}
