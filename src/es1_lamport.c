 ...
 *
 * Evento concorrente:
 *   l'evento interno c di P1 NON e' in relazione causale con a di P0
...
static int clk[N];  /* clk[i] e' il clock corrente del nodo i */

/* Struttura per salvare i timestamp da verificare */
typedef struct {
    int send_p0_p1;   /* C al momento di send(P0->P1) */
 ...
 *
 * Coppie da verificare (i valori sono gia' salvati in saved):
...
static void verify(void) {
    printf("\n=== Verifica proprieta' C(send) < C(recv) ===\n");
    /* TODO */
}
